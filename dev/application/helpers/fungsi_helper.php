...
function is_nav($a, $b){
	...
	// proses insert, update, detail, delete data api pddikti
  	function Api_Hardjo($data = null, $url = null ){
  		global $SConfig;
      	$curl = curl_init();
	...
	function jenis_perangkat(){
		$_this =& get_instance();
		if ($_this->agent->is_browser()){
		    $agent = $_this->agent->browser();
		}elseif ($_this->agent->is_robot()){
		    $agent = $_this->agent->robot(); 
		}elseif ($_this->agent->is_mobile()){
		    $agent = $_this->agent->mobile();
		}else{
			$agent='Other';
		}
	...
	function id_ke_no_resi($id, $kode=''){
		if (!empty($kode)) {
			// TRANSAKSI MASUK
		    $inisial = $kode;
		}else if ($kode == "pencairan"){
			// PENCAIRAN MITRA
		    $inisial = "DTY";
		}else{
			$inisial = "XXX";
	    ...
	    $tgl = date('Ymd');
	    // $tgl = substr($id, 4,6);
    	// $tgl = date_format(date_create($tgl),"Ymd");
    	$num = round(substr($id, 10));
	    $no_nota = $inisial."-".substr(str_shuffle("QWERTYUOPLKJHGFASZXCVBNM"), 0, 3)."-".$tgl."-".$num;
	    return $no_nota;
	}
	...
	// function untuk nama bulan biar indonesia
    ...
    function status_proses_skck($kode){
    	switch ($kode) {
	        case "1":
	            $text = '<span class="badge badge-danger">Masuk</span>';
	            break;
	        case "2":
	            $text = '<span class="badge badge-warning">Proses</span>';
	            break;
	        case "3":
    ...
    function status_proses_sttlp($kode){
    	switch ($kode) {
	        case "1":
	            $text = '<span class="badge badge-info">Masuk</span>';
	            break;
	        case "2":
	            $text = '<span class="badge badge-warning">Proses</span>';
	            break;
	...
	function status_proses_sktlk($kode){
    	switch ($kode) {
	        case "1":
	            $text = '<span class="badge badge-info">Masuk</span>';
	            break;
	...
	function status_proses_sik($kode){
    	switch ($kode) {
	        case "1":
	            $text = '<span class="badge badge-danger">Masuk</span>';
	            break;
